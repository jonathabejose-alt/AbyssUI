...
local Window = WindUI:CreateWindow({
    Title = "AbyssUI (Azure Latch)",
    Author = "Tze",
    Folder = "AbyssUIAzureLatch",
    ...
    User = {
        ...
        Callback = function()
            WindUI:Notify({
                Title = "AbyssUI",
                Content = "Made by Tze | discord: tze0638",
                Icon = "moon",
                Duration = 4,
            })
        end
...
Window:SideBarButton({
    ...
    Callback = function()
        ...
        WindUI:Notify({ Title = "Discord", Content = "Copied invite link to clipboard.", Icon = "check", Duration = 3 })
    end,
})
...
bmSection:Toggle({
    ...
    Callback = function(v)
        if v then
            ...
            mhState.bConn = UserInputService.InputBegan:Connect(function(inp, gp)
...
for _, d in ipairs(dribbleToggles) do
    local tog = autoSection:Toggle({
        ...
        Callback = function(v)
            askState[d.key] = v
            notifySkill(d.title, v and "Enabled." or "Disabled.", 3)
        end,
    })
    dribbleUIToggles[d.key] = tog
end

if not askState.keybindSetup then
    askState.keybindSetup = true
    UserInputService.InputBegan:Connect(function(inp, gp)
        ...
        if m then
            askState[m[1]] = not askState[m[1]]
            if dribbleUIToggles[m[1]] then
                dribbleUIToggles[m[1]]:Set(askState[m[1]])
            end
            notifySkill(m[2], askState[m[1]] and "Enabled." or "Disabled.", 3)
        end
    end)
end
...
RunService.RenderStepped:Connect(function()
    ...
    if not camLockConn then
        camLockConn = RunService.RenderStepped:Connect(function()
            ...
            alCam.CFrame = CFrame.new(alCam.CFrame.Position, activeTarget.Position)
        end)
    end
end)
...
local lineupSegmented = lineSection:Segmented({
    ...
    Callback = function(value)
        ...
        notify("Line Ups", value .. " enabled.", 3)
    end,
})
...
gfSection:Stepper({
    ...
    Callback = function(v)
        ...
        gfStats:Update("Target", tostring(goalFarmTarget))
        notify("Goal Target", "Set to " .. goalFarmTarget, 2)
    end,
})
    ...
    Callback = function(v)
        ...
        if v then
            ...
            task.spawn(function()
                ...
                local startGoals = goals.Value
                while gfState.Enabled do
                    task.wait(1)
...
tzeMoveTab:Button({ Title = "Messi Moveset (Req. Bachira)", Desc = "Dribble, Trap Shot, Riptide, Intercept + Flow. F4 to disable.", Icon = "star", Callback = function() loadstring(game:HttpGet("https://raw.githubusercontent.com/jonathabejose-alt/AbyssUI/refs/heads/main/Messi%20Style.lua", true))() end })
